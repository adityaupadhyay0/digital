...
interface SimulatorState {
  ...
  updateNodeState: (nodeId: string, state: boolean) => void;
}
...
export const useStore = create<SimulatorState>((set, get) => ({
  ...
  addEdge: (edge) => {
    ...
    // Update connected nodes states
  const state = get();
  const lastEdge = state.edges[state.edges.length - 1]; // ✅ Avoid name conflict
  const sourceNode = state.nodes.find(n => n.id === lastEdge.source);
  const targetNode = state.nodes.find(n => n.id === lastEdge.target);

  if (sourceNode?.type === 'logicGate' && targetNode?.type === 'bulb') {
    get().updateNodeState(targetNode.id, sourceNode.data.state || false);
  }
},
  ...
  updateNodeState: (nodeId, state) => set((state) => ({
    nodes: state.nodes.map(node =>
